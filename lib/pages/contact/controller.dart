...
import 'package:chatter/common/entities/contact.dart';
...
class ContactController extends GetxController{
  ...
  Future<void> goChat(ContactItem contactItem) async {
    var from_messages =  await db.collection("message").withConverter(
          ...
          "from_token",isEqualTo: token
      ).where(
          "to_token",isEqualTo: contactItem.token
      ).get();

    print("from messages ${from_messages.docs.isNotEmpty}");

    var to_messages =  await db.collection("message").withConverter(
        fromFirestore: Msg.fromFirestore,
        toFirestore: (Msg msg,options) => msg.toFirestore()
    ).where(
        "from_token",isEqualTo: contactItem.token
    ).where(
        "to_token",isEqualTo: token
    ).get();

    print("to messages ${to_messages.docs.isNotEmpty}");
